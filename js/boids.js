...
const modify_speed_and_direction = () => {
  ...
  for (i = 0; i < agentsAmount; i++) {
    ...
    for (j = 0; j < agentsAmount; j++) {
      if (j == i) continue;
      dist = Math.sqrt(
        Math.pow(boids[i].x - boids[j].x, 2) +
          Math.pow(boids[i].y - boids[j].y, 2)
      );
      deg = Math.acos(
        (boids[i].vx /
          Math.sqrt(boids[i].vx * boids[i].vx + boids[i].vy * boids[i].vy)) *
  ...
  for (i = 0; i < agentsAmount; i++) {
    ...
    boids[i].vx +=
      weightNeighbourVelocity * (boids[i].mean_vx / boids[i].num - boids[i].vx);
    boids[i].vy +=
      weightNeighbourVelocity * (boids[i].mean_vy / boids[i].num - boids[i].vy);
    ...
    for (j = 0; j < agentsAmount; j++) {
      ...
      if (dist < neighbourRadius && deg < observDegree) {
        if (Math.abs(boids[j].x - boids[i].x) > minDistance) {
          boids[i].vx +=
            (weightNeighbourDistance / boids[i].num) *
            (((boids[j].x - boids[i].x) * (dist - boids[i].mean_d)) / dist);
          boids[i].vy +=
            (weightNeighbourDistance / boids[i].num) *
            (((boids[j].y - boids[i].y) * (dist - boids[i].mean_d)) / dist);
        } //neighbours are too close
        else {
          boids[i].vx -=
            (weightMinimalDistance / boids[i].num) *
            (((boids[j].x - boids[i].x) * minDistance) / dist -
              (boids[j].x - boids[i].x));
          boids[i].vy -=
            (weightMinimalDistance / boids[i].num) *
            (((boids[j].y - boids[i].y) * minDistance) / dist -
              (boids[j].y - boids[i].y));
        }
      }
    }
...
const move_and_display = () => {
  ...
  for (i = 0; i < agents.length; i++) {
    ...
    if (boids[i].vx < 0) {
      boids[i].rotate =
        90.0 + (Math.atan(boids[i].vy / boids[i].vx) * 180.0) / Math.PI;
    } else {
      boids[i].rotate =
        -90.0 + (Math.atan(boids[i].vy / boids[i].vx) * 180.0) / Math.PI;
    }
  ...
  for (let i = 0; i < TARGET_POSITIONS.length; i++) {
    if (TARGET_POSITIONS[i].depth == 0) {
      let dist = getDistance(
        ...
        boid_y,
        TARGET_POSITIONS[i].y
      );
      if (dist < width) return i;
    }
    if (
      boid_x >= TARGET_POSITIONS[i].x - TARGET_POSITIONS[i].width / 2 &&
      boid_x <= TARGET_POSITIONS[i].x + TARGET_POSITIONS[i].width / 2 &&
      boid_y >= TARGET_POSITIONS[i].y - TARGET_POSITIONS[i].depth / 2 &&
      boid_y <= TARGET_POSITIONS[i].y + TARGET_POSITIONS[i].depth / 2
    ) {
      return i;
    }
...
const getNumberOfBoidsInArea_bw = (index) => {
  ...
  for (i = 0; i < agents.length; i++) {
    if (TARGET_POSITIONS[index].depth == 0) {
      let dist = getDistance(
        boids[i].x,
        TARGET_POSITIONS[index].x,
        boids[i].y,
        TARGET_POSITIONS[index].y
      );
      if (dist < width) number = number + 1;
    }
    if (
      boids[i].x >=
        TARGET_POSITIONS[index].x - TARGET_POSITIONS[index].width / 2 &&
      boids[i].x <=
        TARGET_POSITIONS[index].x + TARGET_POSITIONS[index].width / 2 &&
      boids[i].y >=
        TARGET_POSITIONS[index].y - TARGET_POSITIONS[index].depth / 2 &&
      boids[i].y <=
        TARGET_POSITIONS[index].y + TARGET_POSITIONS[index].depth / 2
    ) {
      number = number + 1;
...
const setBoidsTargets_bw = (boids) => {
  for (let i = 0; i < agentsAmount; i++) {
    ...
    if (boids[i].current_target_id == -1) {
      ...
      if (Math.floor(Math.random() * chanceToGetToTarget) == 0) {
        // for 3 there is 33.3% chance that agent will move towards defined target
        let targetId;
        do {
          targetId = ~~(Math.random() * TARGET_POSITIONS.length);
        } while (targetId == boids[i].recently_visited_target_id);
        const xDirection =
          boids[i].x - TARGET_POSITIONS[targetId].x > 0 ? -1 : 1;
        const yDirection =
          boids[i].y - TARGET_POSITIONS[targetId].y > 0 ? -1 : 1;
      ...
      const xDirection =
        boids[i].x - TARGET_POSITIONS[boids[i].current_target_id].x > 0
          ? -1
          : 1;
      const yDirection =
        boids[i].y - TARGET_POSITIONS[boids[i].current_target_id].y > 0
          ? -1
          : 1;
      boids[i].vx = xDirection * (Math.random() * speedToTarget); // maxVelocity
      boids[i].vy = yDirection * (Math.random() * speedToTarget);
...
const getNextTarget_bw = (boid) => {
  ...
  while (
    getDistance(
      boid.x,
      TARGET_POSITIONS[startingIndex].x,
      boid.y,
      TARGET_POSITIONS[startingIndex].y
    ) > TARGET_POSITIONS[startingIndex].attraction_range ||
    boid.preferences[startingIndex] == 1 ||
    getNumberOfBoidsInArea_bw(startingIndex) >= crowdThreshold
  ) {
  ...
  let currdist = getDistance(
    boid.x,
    TARGET_POSITIONS[targetId].x,
    boid.y,
    TARGET_POSITIONS[targetId].y
  );
  if (targetId == boid.current_target_id)
    currdist = currdist * currentTargetPriority;
  currdist =
    currdist *
    boid.preferences[j] *
    (0.5 + getNumberOfBoidsInArea_bw(startingIndex) / (2 * crowdThreshold));
  for (let j = startingIndex + 1; j < TARGET_POSITIONS.lenght; j++) {
    if (boid.preferences[startingIndex] < 1) {
      ...
      if (
        dist <= TARGET_POSITIONS[j].attraction_range ||
        crowd < crowdThreshold
      ) {
        if (j == boid.current_target_id) dist = dist * currentTargetPriority;
        dist =
          dist * boid.preferences[j] * (0.5 + crowd / (2 * crowdThreshold));
        if (dist < currdist) {
          currdist = dist;
          targetId = j;
        }
...
const isHittingObstacle = (boid_next_pos_x, boid_next_pos_y) => {
  for (let i = 0; i < OBSTACLE_POSITIONS.length; i++) {
    if (
      boid_next_pos_x + WIDTH >
        OBSTACLE_POSITIONS[i].x - OBSTACLE_POSITIONS[i].width / 2 &&
      boid_next_pos_x - WIDTH <
        OBSTACLE_POSITIONS[i].x + OBSTACLE_POSITIONS[i].width / 2 &&
      boid_next_pos_y + WIDTH >
        OBSTACLE_POSITIONS[i].y - OBSTACLE_POSITIONS[i].depth / 2 &&
      boid_next_pos_y - WIDTH <
        OBSTACLE_POSITIONS[i].y + OBSTACLE_POSITIONS[i].depth / 2
    ) {
      return true;
...
const getNextTarget = (boid, index) => {
  ...
  const targetDistances = TARGET_POSITIONS.map(
    (_target) =>
      ~~getDistance(boid.x, _target.x, boid.y, _target.y) -
      _target.attraction_range
  );
  ...
  const distanceToTarget =
    getDistance(
      ...
      boid.y,
      TARGET_POSITIONS[targetID].y
    ) - TARGET_POSITIONS[targetID].attraction_range;
  const isMovingToTarget = distanceToTarget < 0;
  const isNotAlreadyVisited =
    boid.recently_visited_target_id.indexOf(targetID) === -1;
...
const initSecondAlgo_bw = (scene) => {
  ...
  for (i = 0; i < agentsAmount; i++) {
    ...
    boids[i].current_target_id =
      ~~(Math.random() * (TARGET_POSITIONS.length + 1)) - 1;
    boids[i].preferences = new Array(TARGET_POSITIONS.length).fill(
      Math.random()
    );
  }
  intervalID = setInterval(() => {
